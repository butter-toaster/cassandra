	...
	if canpress == true:
		...
		match talknumber:
			...
			2:
				...
				%break.hide()
				$".."._type("HELP ME-", %textleft, 1)
				$".."._talk(%womantalksfx)
				await get_tree().create_timer(0.05).timeout
				%gunshots.play()
				%textleft.modulate = Color("7547bdff")
				%textleft.text = "HELP M"
...
func _bargingsequence() -> void:
	...
	policetween.tween_property(%officer1, "global_position:x", -300, 1.3)
	policetween.tween_property(%officer2, "global_position:x", -300, 1.9)
	policetween.tween_property(%officer3, "global_position:x", -300, 2.6)
	policetween.tween_property(%endingnoisesfx,"volume_db", -10, 3)
